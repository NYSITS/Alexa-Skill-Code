...
Originally Created: June 27, 2017

Modified: 

Author: Nicholas Stucchi, Nicholas.Stucchi@its.ny.gov
'''
...
def on_intent(intent_request, session):
    ...
    if intent_name == "AMAZON.HelpIntent":
        ...
        if previous_place == "difficulty menu":
            ...
            try:
                answer_given = int(intent["slots"]["NumberTrueFalse"]["value"])
            except ValueError:
...
def get_welcome_response(request, session):
    ...
    card_title = "Welcome to Practice Driver Permit."
    ...
    reprompt_text = "<speak>Would you like to practice?  </speak>"
...
def play_game(intent, session):
    """
    Main function for Practice Driver Permit
    """
    ...
    if "last_question" in game_details.keys():
        ...
        try:
            if 'Number' in intent['slots']:
                answer_given = intent["slots"]["Number"]["value"]
            else:
    ...
    if game_details["question_count"] < session["attributes"]["round_length"]:
        ...
        if not last_question:
            ...
            card_title = "Let's start a a practice test!"
        else:
        ...
        if grade > .80:
            ...
            if new_difficulty == 0: new_difficulty = "easy"
            if new_difficulty == 1: new_difficulty = "medium"
            if new_difficulty == 2: new_difficulty = "hard"
            session["attributes"]["difficulty"] = new_difficulty
            result_text += "You did so well, I'm now setting the difficulty level to %s.  " % (new_difficulty)
        else:
            result_text += "You need a bit more practice, I'm leaving your difficulty level at %s.  " % session["attributes"]["difficulty"]
        ...
        card_title = "Practice Test Round Over"
...
def get_help(intent, session):
    ...
    current_difficulty = session["attributes"]["difficulty"]
    ...
    card_title = "Practice Driver Permit Help"
    text = "Looking for help?  Practice Driver Permit is easy to use.  " \
           ...
           "The current difficulty level is set to " + current_difficulty + ".  " \
...
def difficulty_menu(intent, session):
    ...
    card_title = "Practice Driver Permit Difficulty Settings"
...
def set_difficulty(intent, session):
    ...
    if 'Number' in intent['slots']:
        ...
        text = "The difficulty is now set to %s.  " % (difficulty)
    else:
        text = "I couldn't tell what difficulty level you asked for, it remains set at %s.  " % (session["attributes"]["difficulty"])
...
def set_roundlength(intent, session):
    ...
    if 'Number' in intent['slots']:
        ...
        text = "The round length is now set to %s questions.  " % (round_length)
    else:
        text = "I couldn't tell what round length you asked for, it remains set at %s questions.  " % (session["attributes"]["round_length"])

    card_title = "Practice Driver Permit Round Length Setting"
    speech_output = "<speak>" \
                    + text + \
                    "<break time=\"500ms\" />" \
                    "Would you like to start a game?  " \
...
def get_question(difficulty):
    ...
    if operation["operation"] == "question_1":
        answer = term_2
        question_text = ("%s." % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_2":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_3":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_4":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_5":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_6":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_7":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_8":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_9":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
    if operation["operation"] == "question_10":
        answer = term_2
        question_text = ("%s" % (term_1))
        print("The correct answer is %s" % (answer))
...
def setup_session_attributes():
    '''
    Sets up initial Practice Driver Permit Session Attributes if new session.
    '''
    session_attributes = {}
    ...
    [
        {
            ...
            "operations": [
                {
                    "operation": "question_1",
                    "term_count": 2,
                    "question": "You must press on the accelerator if your tire goes flat while driving.",
                    "answer": 2
                },
                {
                    "operation": "question_2",
                    "term_count": 2,
                    "question": "You must drive 25 miles per hour or less if there is no posted speed limit in New York City.",
                    "answer": 1
                },
                {
                    "operation": "question_3",
                    "term_count": 2,
                    "question": "You should avoid eye contact when confronted by an aggressive driver.",
                    "answer": 1
                },
                {
                    "operation": "question_4",
                    "term_count": 2,
                    "question": "You should increase your speed when a driver tailgates you.",
                    "answer": 1
                },
                {
                    "operation": "question_5",
                    "term_count": 2,
                    "question": "The two second rule is used to maintain a good space cushion.",
                    "answer": 1
                },
                {
                    "operation": "question_6",
                    "term_count": 2,
                    "question": "The three point turn method is used to turn around on a narrow, two way street.",
                    "answer": 1
                },
                {
                    "operation": "question_7",
                    "term_count": 2,
                    "question": "You are required to signal with directional lights or hand signals 50 feet ahead.",
                    "answer": 2
                },
                {
                    "operation": "question_8",
                    "term_count": 2,
                    "question": "You must not pass a vehicle on the right if the vehicle ahead is making a right turn.",
                    "answer": 1
                },
                {
                    "operation": "question_9",
                    "term_count": 2,
                    "question": "Service signs are yellow and diamond shaped with black lettering or symbols.",
                    "answer": 2
                },
                {
                    "operation": "question_10",
                    "term_count": 2,
                    "question": "Moving from one lane to another is not defined as changing lanes.",
                    "answer": 2
                }
            ]
        },
