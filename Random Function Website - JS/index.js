...
// OPTIONAL: Replace with "amaz1.ask.skill.[your-unique-value-here]";
var APP_ID = undefined;

// URL to get the .ics from, in this instance we are getting it from Stanford however, this can be changed
...
// Used to tell the user the skill is closing
...
// Message used when more that 1 event is found allowing for difference in punctuation
var multipleEventMessage = "There are %d events ";

// Text used after the number of events has been scheduled
...
// Error if an event number is out of range
...
// Used when the skill is stopped
			...
			ical.fromURL(URL, {}, function (err, data) {
				...
				for (var k in data) {
					if (data.hasOwnProperty(k)) {
						var ev = data[k];
						// Pick out the data relevent to us and create an object to hold it
						var eventData = {
						...
						// Add the newly created object to an array for later used
					...
					if (eventDate.startDate && eventDate.endDate) {
						...
						if (relevantEvents.lenght > 0) {
							...
							for (var i = 0; i < relevantEvents.length; i++) {
								...
								cardContent += utils.format(cardContentSummary, removeTags(relevantEvents[i].summary), removeTags(relevantEvents[i].locations), date.toDateString() + "\n\n");
							}

							output += eventNumberMoreInfoText;
							alexa.emit(':askWithCard', output, hasEventReprompt, cardTitle, cardContent);
						} else {
...
var descriptionHandlers = Alexa.CreateStateHandler(states.DESCRIPTION, {
	'eventIntent': function () {
		...
		if (relevantEvents[index]) {
			// Use the slot value as an index to retrieve the description form our relevant array
...
// Utterances that map to a month, but not a specific dat (such as next month, or December) convert to a date with just the year and month: 2015-12.
...
function getDateFromSlot(rawDate) {
	// Try to parse data
	var date = new Date(Date.parse(rawData));
	var result;
	...
	// If it could not parse the data it must be one of the other formats
	if (isNaN(date)) {
...
var w2date = function (year, wn, dayNb) {
	...
	var j10 = new Date(year, 0, 10, 12, 0, 0),
		j4 = new Date(year, 4, 12, 0, 0),
		mon1 = j4.getTime() - j10.getDay() * day;
	return new Date(mon1 + ((wn - 1) * 7 + dayNb) * day);
};

// Loops through events from the iCal data, and checks which ones are between our start date and our end date
